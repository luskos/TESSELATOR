// ============================================================================
// MORT_TESSELATOR_CORE.cpp - FINAL WORKING VERSION
// ============================================================================
...
// ============================================================================
// TESSELATOR CORE - SINGLE DEFINITION
// ============================================================================
...
public:
    ...
    void tesselate() {
        ...
        for (int i = 0; i < TOTAL_CELLS; i++) {
            ...
            // Balanced scaling for visible growth
            double harvested = probability * MortConstants::QUANTUM_ENERGY * 1e9;
        ...
        local_compute = ops * MortConstants::LANDAUER_LIMIT * 1e-3;
        ...
        static std::uniform_real_distribution<double> noise(-0.001, 0.001);
        harvested_energy.store(harvested_energy.load() + noise(gen) * local_harvest);
    }
...
public:
    ...
    double getTotalEnergy() const {
        return totalSystemEnergy.load();
    }
    
    bool isGridConnected() const {
        return gridConnected.load();
    }
    ...
    void housekeepingLoop() {
        ...
        while (gridConnected.load()) {
            ...
            double systemBalance = netEnergy * 1000 + (gpuPower * 0.1);
            ...
            if (counter++ % 10 == 0) {
                ...
                std::cout << "\rCOP: " << cop << " | Total Energy: " << totalSystemEnergy.load() 
...
╔═══════════════════════════════════════════════════════════════════════════╗
║                                                                           ║
║   MORT TESSELATOR v1.0 - REAL ENERGY MONITOR                             ║
║                                                                           ║
╚═══════════════════════════════════════════════════════════════════════════╝
)" << std::endl;
    ...
    std::thread computeThread([&]() {
        ...
        while (running.load()) {
            ...
            auto now = std::chrono::steady_clock::now();
            double display_dt = std::chrono::duration<double>(now - lastDisplay).count();
            
            if (display_dt >= 0.1) {
                double totalTime = std::chrono::duration<double>(now - start).count();
                double opsPerSec = static_cast<double>(operations) / totalTime;
                
                std::cout << "\r⚛️  Ops: " << operations/1000000 << "M | "
                          << "Rate: " << opsPerSec/1e9 << " GOPS/s | "
                          << "Energy: " << netEnergy.load() << " J | "
                          << "Grid: " << (sustenance.isGridConnected() ? "ON " : "OFF") << " | "
                          << "COP: " << tesselator.getCOP() << "    "
                          << std::flush;
    ...
    std::cout << "\n═══════════════════════════════════════════════════════════" << std::endl;
    std::cout << "Reactor shutdown complete." << std::endl;
    std::cout << "Final reactor energy: " << netEnergy.load() << " J" << std::endl;
    std::cout << "Final system energy: " << sustenance.getTotalEnergy() << " J" << std::endl;
    std::cout << "═══════════════════════════════════════════════════════════" << std::endl;
